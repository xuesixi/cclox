##
在增加了一个新的指令后，需要新增
* disassembler中的指令enum对应的字符串名字
* 如果是token的表达式，那么增加pratt三表的内容。
* vm中对应的处理逻辑

## makefile

由於我希望clox 最終是一個單獨的文件，所以我將標准庫嵌入到了 clox 可執行文件中。這導致了循環依賴的問題：

* lox 標准庫被 clox 編譯為字節碼
* 字節碼被轉化為頭文件
* 這些頭文件被 clox include。

為了避免這些問題，我使用了如下的方案：

* 一開始，手動創建幾個lox 標准庫頭文件的空文件，作為佔位符。
* 這些空的標准庫頭文件會在第一次 clox 中構建中被 include。由於它們是空的，此時產生的 clox 解釋器 實際上並不具有真正的標准庫。但是，它已經具備了完備的編譯能力。
* 使用此時的 clox，重新編譯標准庫，產生真正的標准庫頭文件。
* 重新構建 clox，這一次，真正的標准庫文件會被嵌入到最終的解釋器中。
